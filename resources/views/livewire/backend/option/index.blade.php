...
<div>
    <section class="container-fluid py-4">
            ...
            <div class="card-body">
                ...
                @if($option_group_name)
                    <div class="alert alert-info py-2 px-3 d-inline-flex align-items-center gap-2 mb-3 rounded-pill small">
                        ...
                        <span>Filtering Group: <strong>{{ $option_group_name }}</strong></span>
                    </div>
                @endif
                ...
                <div class="d-flex align-items-center justify-content-between bg-light rounded px-3 py-2 mb-3 text-muted small">
                    ...
                    <div>
                        <span>Current Page: <strong>{{ $options->currentPage() }}</strong></span>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($options as $key => $val)
                                <tr>
                                    ...
                                    <td class="text-dark small">{{ str_replace('_', ' ', $val->option_group_name) }}</td>
                                    <td class="fw-semibold text-dark">{{ str_replace('_', ' ', $val->option_value) }}</td>
                                    <td class="text-muted small">{{ str_replace('_', ' ', $val->option_value2) ?: '-' }}</td>
                                    <td class="text-muted small">{{ str_replace('_', ' ', $val->option_value3) ?: '-' }}</td>
                                    <td>
            ...
            <div class="card-footer bg-white border-top py-3">
                <div class="row align-items-center g-3">
                    ...
                    <div class="col-md-6 text-md-end text-muted small">
                        <p class="mb-0">
                            {{ __('Showing') }} <span class="fw-semibold text-dark">{{ $options->firstItem() }}</span>
                            {{ __('to') }} <span class="fw-semibold text-dark">{{ $options->lastItem() }}</span>
                            {{ __('of') }} <span class="fw-semibold text-dark">{{ $options->total() }}</span> {{ __('results') }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        @include('livewire.backend.option.create')
        @include('livewire.backend.option.edit')
    </section>
</div>
