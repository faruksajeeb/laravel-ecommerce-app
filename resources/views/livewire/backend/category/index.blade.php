        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                ...
                <div class="d-flex align-items-center justify-content-between bg-light rounded px-3 py-2 mb-3 text-muted small">
                    ...
                    <div>
                        <span>Current Page: <strong>{{ $categories->currentPage() }}</strong></span>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($categories as $key => $val)
                                <tr>
                                    ...
                                    <td>
                                        @if($val->image)
                                            <img src="{{ asset('frontend-assets/imgs/categories/' . $val->image) }}" 
                                                 alt="{{ $val->name }}" 
            ...
            <div class="card-footer bg-white border-top py-3">
                <div class="row align-items-center g-3">
                    ...
                    <div class="col-md-6 text-md-end text-muted small">
                        <p class="mb-0">
                            {{ __('Showing') }} <span class="fw-semibold text-dark">{{ $categories->firstItem() }}</span>
                            {{ __('to') }} <span class="fw-semibold text-dark">{{ $categories->lastItem() }}</span>
                            {{ __('of') }} <span class="fw-semibold text-dark">{{ $categories->total() }}</span> {{ __('results') }}
                        </p>
                    </div>
                </div>
            </div>
