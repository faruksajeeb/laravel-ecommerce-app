<x-app-layout>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-3 border-bottom">
        <div class="mb-3 mb-md-0">
            <h4 class="text-dark fw-bold mb-1">
                <i class="fa fa-pencil-alt text-primary me-2"></i>Edit Role — <span class="text-secondary fw-semibold">{{ ucwords($roleInfo->name) }}</span>
            </h4>
            <p class="text-muted small mb-0">Modify access thresholds, updates, or refine capabilities for this profile.</p>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 bg-transparent py-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none link-secondary">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ url('roles') }}" class="text-decoration-none link-secondary">Roles</a></li>
                <li class="breadcrumb-item active text-primary fw-medium" aria-current="page">Edit</li>
            </ol>
        </nav>
    </div>

    <form action="{{ route('roles.update', Crypt::encryptString($roleInfo->id)) }}" method="POST" id="roleEditForm">
        @method('PUT')
        @csrf
        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <div class="card border border-light-subtle shadow-sm rounded-4 p-4 sticky-lg-top" style="top: 1.5rem; z-index: 10;">
                    ...
                    <div class="mb-4">
                        <label for="role_name" class="form-label fw-semibold text-secondary small @if($errors->has('name')) text-danger @endif">ROLE NAME</label>
                        <input type="text" name="name" id="role_name" 
                               value="{{ old('name', $roleInfo->name) }}" 
                               class="form-control form-control-lg border-2 shadow-none fs-6 px-3 @error('name') is-invalid @enderror" 
                    ...
                    <button type="submit" class="btn btn-primary btn-lg w-100 py-2.5 fs-6 fw-semibold shadow-sm rounded-3">
                        <i class="fa fa-save me-2"></i>Update Configuration
                    </button>
                </div>
            </div>
                        ...
                        @foreach ($permission_groups as $groupIndex => $permission_group)
                            ...
                            <div class="row g-0 border-bottom border-light-subtle align-items-start permission-group-row">
                                ...
                                <div class="col-md-4 bg-light bg-opacity-25 p-4 border-end border-light-subtle h-100">
                                    <div class="form-check form-switch group-permission-switch" data-group="{{ $permission_group->group_name }}">
                                        <input type="checkbox" name="group-permission[]" id="group_{{ $groupIndex }}" 
                                               class="form-check-input group-input cursor-pointer" 
                                               {{ App\Models\User::roleHasPermissions($roleInfo, $groupWisePermissions) ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold text-dark cursor-pointer" for="group_{{ $groupIndex }}">
                                            ...
                                            <div class="col-sm-6">
                                                <div class="p-2.5 px-3 border border-transparent rounded-3 hover-bg duration-150">
                                                    <div class="form-check single-permission-box" 
                                                         data-group-class="per-{{ $permission_group->group_name }}" 
                                                         data-group-name="{{ $permission_group->group_name }}" 
                                                         data-count="{{ $permissinCount }}">
                                                        
                                                        <input type="checkbox" value="{{ $permission->name }}" name="permissions[]" 
                                                               id="permission{{ $permission->id }}" 
                                                               class="form-check-input perm-input cursor-pointer" 
                                                               {{ $roleInfo->hasPermissionTo($permission->name) ? 'checked' : '' }}>
                                                        <label class="form-check-label text-secondary cursor-pointer fs-7" for="permission{{ $permission->id }}">
    ...
    @push('scripts')
        <script>
            $(function() {
                // Initialize setup state on bootup
                allChecked();
                ...
                function allChecked() {
                    const totalPermissionsCount = {{ count($permissions) }};
